use crate::{
    admin::backend::get_proxies, AdminProxyBody, ProxyData, ProxyHandle, ThreadMessage, Traffic,
};
...
async fn live_proxies_init(
    live_proxies: &HashMap<u16, ProxyData>,
    proxies: Arc<RwLock<HashMap<u16, ProxyHandle>>>,
    ...
    for (m_port, proxy_data) in live_proxies {
        ...
        if let Err(err) =
            crate::run_server(*m_port, proxy_data, Vec::new(), proxies.clone(), tx.clone()).await
        {
            error!("Cannot restart proxy on {m_port}. Error: {err}");
        }
    }
...
pub async fn start_admin_thread(
    ...
    let admin = task::spawn(async move {
        ...
        let mut live_proxies: HashMap<u16, ProxyData> = match get_proxies(url.clone()).await {
