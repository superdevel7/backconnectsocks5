...
where
    ...
    CopyBidirectional {
        a,
        b,
        a_to_b: TransferState::Running(CopyBuffer::new(delay.clone())),
        b_to_a: TransferState::Running(CopyBuffer::new(delay.clone())),
    }
    .await
}
...
impl CopyBuffer {
    fn new(delay: Arc<AtomicU64>) -> Self {
        Self {
            read_done: false,
            need_flush: false,
            pos: 0,
    ...
    {   
        loop {
            ...
            let delay = self.delay.load(std::sync::atomic::Ordering::Acquire);
